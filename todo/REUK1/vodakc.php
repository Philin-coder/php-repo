...
?>  
    ...
    <div class="page-title-img">
        <img class="img-full" alt="page title img" src="assets/images/headers/3518437237145495_e790.jpg">
    	...
    	<div class="container">
			...
			<form  enctype='multipart/form-data' action="" method="POST"   class="form-inline">
				...
				<div class=" mb-2 col-4">
					<input type="text" class="form-control"  name="naz"  placeholder="Введите название" requered>
				</div>
				<div class="mb-2 col-4">
					<textarea cols="60" rows="2" class="form-control"  name="opis"  placeholder="Введите описание" requered></textarea>
				</div>
				<div class="mb-2 col-4">
...
<hr></hr>
<form action=""  method="post"><h2 align="center">Удаление акции </h3><br></br>
<div class="form-group mb-2 col-4">
					<select class="form-control" name='id_ak[]' multiple ><br><br>   
					<?
					$sql = mysqli_query($link,"select * from akzii");
					while ($row = mysqli_fetch_array($sql)) {
						
					echo'<option value="'.$row['id_akc'].'">'.$row['naz'].'</option>';}	
...
<?
if ( isset ( $_POST['id_ak'] ) )
{
  $ids = implode( ',', $_POST['id_ak'] );
  $query = 'DELETE FROM akzii WHERE id_akc IN ('.$ids.')';
 $result= mysqli_query( $query );?>
<meta http-equiv='Refresh' content='3; url=vodakc.php />;
<? /* exit; */
}
if ($query == 'true') {
echo " <strong><allign='center'>Акция удалена</strong>";}?>
...
// Текстошибок
$error_by_mysqli = "<labelclass=\"label\">Ошибкапридобавленииданныхвбазу</span>";
...
$query = mysqli_query($link,"INSERT INTO akzii(naz, opis,zena,img) VALUES ('".$naz."','".$opis."', '".$zena."','".$foto_name."')") or die("Ошибкадобавлениябукетов");
/* $result = mysqli_query($query); */
if ($query  == 'true') {
echo " <strong><allign='center'>Акция добавлена</strong>";
}
// В противном случае, выводим ошибку при добавлении в базу данных
else {
echo "$error_by_mysqli";}}}?><br></br><br></br><br><br>
	...
	<div class="row">
		<table class="table">
			...
			<?php
			$sql = "select id_akc,naz, opis,zena, img from akzii";
		$result= mysqli_query($link, $sql) or die(mysqli_error($link));
			while ($row = mysqli_fetch_array($result)){			
	echo '<tr><td>'.$row['id_akc'].'</td><td>'.$row['naz'].'</td><td>'.$row['opis'].'</td><td>'.$row['zena'].'</td><td>'.$row['img'].'</td></tr>';
    }
