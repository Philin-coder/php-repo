...
<div class="form-group mb-2 col-4">
					...
					echo'<option value="'.$row['id_pizza'].'">'.$row['nazvanie'].'">'.$row['zena'].'</option>';}	
					
					?>
					</select>
				</div>
				<div class="form-group mb-2 col-4">
					<input type="text" class="form-control"  name="название"  placeholder="Введите название" requered>
				</div>
				<div class="form-group mb-2 col-4">
					<input type="text" class="form-control"  name="цена"  placeholder="Введите цену" requered>
				</div>
	...
	if (!empty($_POST)) 
{
	...
	$zena=$_POST['zena'];
	$sql =mysqli_query ($link,"insert into pizza (id_pizza, nazvanie,zena) values ('$id_pizza', '$nazvanie','$zena')");
...
<div class="container">
	<div class="row">
		<table class="table">
			...
			<?php
			$sql = "select id_pizza,nazvanie, zena from pizza";
		$result= mysqli_query($link, $sql) or die(mysqli_error($link));
			while ($row = mysqli_fetch_array($result)){			
	echo '<tr><td>'.$row['id_pizza'].'</td><td>'.$row['nazvanie'].'</td><td>'.$row['zena'].'</td></tr>';
    }
