...
<em></em>
    ...
    <section>
		...
		<table class="table">
			...
			<?php
			$sql = "select id_zak,id_pizza, adres, tel,data, summa_p,fio,kol_vo,id_user from zakaz";
		$result= mysqli_query($link, $sql) or die(mysqli_error($link));
			while ($row = mysqli_fetch_array($result)){			
	echo '<tr><td>'.$row['id_zak'].'</td><td>'.$row['id_pizza'].'</td><td>'.$row['adres'].'</td><td>'.$row['tel'].'</td><td>'.$row['data'].'</td><td>'.$row['summa_p'].'</td><td>'.$row['fio'].'</td><td>'.$row['kol_vo'].'</td><td>'.$row['id_user'].'</td></tr>';
    }
