...
<div class="page-loader-text">Загрузка</div></div></div><!-- #page-loader -->
...
<li>
<a href="#section-menu" class="scroll-to"></a></li>
<li>
<a href="#section-menu" class="scroll-to">
<form   action="" method="POST"   class="form-inline">
<div class=" mb-2 col-4">
<input type="text" class="form-control"  name="naz"  placeholder="Введите название" requered></div>
<div class="mb-2 col-4">
<input type="text" class="form-control"  name="opis"  placeholder="Введите описание" requered></div>
<div class="mb-2 col-4">
<input type="text" class="form-control"  name="zena"  placeholder="Введите цену" requered></div>						
<div class=" mb-2 col-4">
<button type="submit" class="btnbtn-primary">Добавить</button></div></form></div></div>
<?php
	...
	if (!empty($_POST)) 
{
	...
	$opis=$_POST['opis'];
	$zena=$_POST['zena'];
	$sql =mysqli_query ($link,"insert into akzii (naz, opis,zena) values ('$naz', '$opis','$zena')");
...
<?php
$sql = "select id_akc,naz, opis,zena from akzii";
$result= mysqli_query($link, $sql) or die(mysqli_error($link));
while ($row = mysqli_fetch_array($result)){			
echo '<tr><td>'.$row['id_akc'].'</td><td>'.$row['naz'].'</td><td>'.$row['opis'].'</td><td>'.$row['zena'].'</td></tr>';
}
mysqli_free_result($result);   
mysqli_close($link);
?></table></div></div></a></li></div><!-- #slider-specials -->
</div><!-- .container --></section>  </div><!-- .page-title-img -->
<section id="cart">
<div class="container">
<div class="cart-content">
<div class="cart-close cart-trigger"><i class="fa fa-close"></i></div>
<div class="border-lines-container">
<h1 class="no-top-margin border-lines">Cart</h1></div>
                <form>
